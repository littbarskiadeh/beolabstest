...
import MetaMaskConnection from "../wallet/MetaMaskConnection";
...
const Play = () => {
  ...
  const [walletModalIsOpen, setWalletModalIsOpen] = useState(false); // New state for Wallet Modal
  ...
  const walletOpenModal = () => {
    setWalletModalIsOpen(true); // Open wallet modal
    playClickSound();
  };

  const walletCloseModal = () => {
    setWalletModalIsOpen(false); // Close wallet modal
    playClickSound();
  };
  ...
  return (
    ...
    >
      ...
      <div className="button-container">
        ...
        <button
          className={`game-button ${isCalmMode ? "calm-button" : ""}`}
          onClick={walletOpenModal} // Trigger wallet modal
          onMouseEnter={() => hoverAudioRef.current.play()}
        >
          CONNECT TO WALLET
        </button>
      </div>
